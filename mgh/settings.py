...
def default_data_dir() -> Path:
    ...
    legacy = Path(__file__).resolve().parents[2] / "mgh-test"
    if legacy.is_dir():
        # Preserve existing sessions while this package still lives under the
        # MAGI Memo parent repository. Fresh checkouts and wheels use the
        # user-scoped directory below.
        return legacy
    # Keep runtime state outside the source tree and site-packages. This makes
    # editable checkouts and installed wheels follow the same storage contract.
    return (Path.home() / ".magi-harness").resolve()
